...
class EnrollmentController extends Controller
{
     ...
     * require an admin grant until card checkout is wired (Plan §9.1).
     */
    public function store(Request $request, Course $course): RedirectResponse
    {
        ...
        // Paid access is granted by an admin until Stripe/PayHere plugs in.
        if (! $course->isFree()) {
            return redirect()
                ->route('courses.show', $course->slug)
                ->with('info', 'Paid access is granted by an admin. Open a support ticket to request enrollment.');
        }
